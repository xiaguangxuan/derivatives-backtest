...
# data = data[data['日期'] > '2010-01-01']
# timestamp = sorted(list(set(data['日期'])))
# data = data.set_index('日期')
# # data['年月'] = data['日期'].apply(lambda x: x[:-3].replace('-', ''))
# value = data.groupby('代码')[['成交额']].rolling(20).mean()
# high = data.groupby('代码')[['最高']].rolling(20).max()
# low = data.groupby('代码')[['最低']].rolling(20).min()
# data = high.join(low)
# highest = data['最高'].groupby('代码').rolling(240, min_periods = 60).max()
# lowest = data['最低'].groupby('代码').rolling(240, min_periods = 60).min()
...
# trade_date = w.tdays('2010-01-01', "2023-08-23", "Period=W", usedf=0).Data[0]
# for index_code in index_list[0:1]:
...
#     pool_information = pd.read_excel('./data/申万行业成分进出记录.xlsx', sheet_name = index_code)
...
#             flag = max(t[i].strftime('%Y-%m-%d'), '2012-01-01')
...
#     for i in range(52, len(last_trade_date)):
...
#         sample_year_data = data[(data['代码'].isin(sample_pool)) & (data['日期'] >= last_trade_date[i - 52]) & (data['日期'] <= last_trade_date[i])]
        
#         sample_month_data = data[(data['代码'].isin(sample_pool)) & (data['日期'] >= last_trade_date[i - 4]) & (data['日期'] <= last_trade_date[i])]

#         sign1 = sample_month_data['最低'].values == sample_year_data.groupby(['代码'])['最低'].min().values
#         NewLowNum = np.sum(sign1)
#         # 可以对计算 max 的数据再次切片，剔除已经是 min 的标的
#         sign2 = sample_month_data['最高'].values == sample_year_data.groupby(['代码'])['最高'].max().values
...
#     mkt = w.wsd(index_code, "close", flag, "2023-08-20", "Period=M;Fill=Previous", usedf=True)[1]
