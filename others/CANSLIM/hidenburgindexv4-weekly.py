...
# value = data.groupby('代码')[['成交金额(元)']].rolling(4).mean()
# high = data.groupby('代码')[['最高价(元)']].rolling(4).max()
# low = data.groupby('代码')[['最低价(元)']].rolling(4).min()
# data = high.join(low)
# highest = data['最高价(元)'].groupby('代码').rolling(52, min_periods = 12).max()
# lowest = data['最低价(元)'].groupby('代码').rolling(52, min_periods = 12).min()
...
# trade_date = w.tdays('2010-01-01', "2023-09-08", "Period=W", usedf=0).Data[0]
...
#         # sample_year_data = data[(data['代码'].isin(sample_pool)) & (data['日期'] >= last_trade_date[i - 51]) & (data['日期'] <= last_trade_date[i])]
        
#         # sample_month_data = data[(data['代码'].isin(sample_pool)) & (data['日期'] >= last_trade_date[i - 3]) & (data['日期'] <= last_trade_date[i])]

#         # sign1 = sample_month_data.groupby(['代码'])['最低价(元)'].min().values == sample_year_data.groupby(['代码'])['最低价(元)'].min().values
#         # NewLowNum = np.sum(sign1)
#         # # 可以对计算 max 的数据再次切片，剔除已经是 min 的标的
#         # sign2 = sample_month_data.groupby(['代码'])['最高价(元)'].max().values == sample_year_data.groupby(['代码'])['最高价(元)'].max().values
...
#         sign1 = sample_year_data['12月最低'].values == sample_year_data['最低价(元)'].values
#         NewLowNum = np.sum(sign1)
#         # 可以对计算 max 的数据再次切片，剔除已经是 min 的标的
#         sign2 = sample_year_data['12月最高'].values == sample_year_data['最高价(元)'].values
#         NewHighNum = np.sum(sign2) - np.sum(sign1 & sign2)
...
#     plt.close()
#     Database_Hindenburg[index_code] = HindenburgIndex
